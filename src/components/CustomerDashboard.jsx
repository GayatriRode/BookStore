...
const CustomerDashboard = () => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await axios.get(`https://bookstore-backend-v7jm.onrender.com/books?userId=${userId}`);
  ...
  const handleBuyBook = async (book) => {
    try {
      ...
      if (copies) {
        const response = await axios.post(`https://bookstore-backend-v7jm.onrender.com/purchase`, { userId, bookId: book._id, copies });
  ...
  const handleLogout = async () => {
    try {
      await axios.post('https://bookstore-backend-v7jm.onrender.com/logout', { userId });
        ...
        <div className="fixed top-0 left-0 w-full h-full bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg overflow-hidden shadow-xl max-w-lg w-full p-4">
            ...
            <div className="p-4">
              ...
              <p className="text-sm text-gray-600">{selectedBook.publication}</p>
